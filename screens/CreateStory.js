...
export default class CreateStory extends Component {
  ...
  async addStory() {
    if (this.state.title && this.state.description && this.state.story && this.state.moral) {
      ...
      await firebase.database().ref("/post/"+Math.random().toString(36).slice(2)).set(storyData)
      .then(function(snapshot){})
      this.props.setUpdateToTrue()
      this.props.navigation.navigate('feed')
    } else {
      Alert.alert('error', "one or more fields aren't filled", [{
        text: "okay",
        onPress: ()=>console.log('okayPressed')
      }], {cancellable: false})
    }
  }
  fetchUser = ()=> {
            ...
            <ScrollView>
              ...
              <View style={{ height: RFValue(this.state.dropdownHeight) }}>
                <DropDownPicker
                  ...
                  labelStyle={
                    this.state.lightTheme ? styles.dropdownLabelLight : styles.dropdownLabel
                  }
                  arrowStyle={
                    this.state.lightTheme ? styles.dropdownLabelLight : styles.dropdownLabel
                  }
